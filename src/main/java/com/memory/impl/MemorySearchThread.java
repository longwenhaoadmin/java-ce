	...
	public void run()
	{
		try 
		{
			...
			if(searchType==0)
			{
				...
				//进程ID
				long pid = mainWnd.currentProcess.getPid();
				...
				//搜索的开始地址
				long startAddress = Long.decode(mainWnd.memoryStartAddress.getText().trim());
				//搜索的结束地址
				long endAddress = Long.decode(mainWnd.memoryEndAddress.getText().trim());
				//更新TABLE列表
